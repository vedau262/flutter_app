  ...
  RemoteBloc() {
    ...
    eventController.stream.listen((RemoteEvent event) {
      ...
      if (event is IncrementEvent) {
        ...
        state = RemoteState(state.volume + event.increment<=100 ? state.volume + event.increment : 100);
      } else if (event is DecrementEvent) {
        // xử lý giảm âm lượng
        state = RemoteState(state.volume - event.decrement>=0 ? state.volume - event.decrement : 0);
      } else {
        // xử lý mute
        state = RemoteState(0);
      }
